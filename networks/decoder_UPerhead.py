...
class UPerNet(nn.Module):
    ...
    def __init__(self, num_class=2, fpn_dim=256):
    ...
    def forward(self, image, x):
        seg_size = image.shape[2:]
        #resnet_features = self.backbone(x)
        #resnet_features = self.backbone.forward_features_seg(x)
        resnet_features = {
            'resnet_layer1': x[0],
            'resnet_layer2': x[1],
            'resnet_layer3': x[2],
        ...
        #pred = F.softmax(pred.float(), dim=1)  # (bs, num_class, h, w)
        return pred
        
        #return {'fuse': pred}
    
    # def do_eval_forward(self, pred):
    #     with torch.no_grad():   
    #         pred_segs = F.softmax(pred['fuse'].float(), dim=1)  # (bs, num_class, h, w)
    #     return pred_segs
...
if __name__ == "__main__":
    img = torch.rand(8, 3, 224, 224).float()
    feats = tuple([torch.rand(8, 96, 56, 56), torch.rand(8, 192, 28, 28), torch.rand(8, 384, 14, 14), torch.rand(8, 768, 7, 7)])
    net = UPerNet()
    out = net(img, feats)
    print(out.shape)