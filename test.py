...
from UltraUPConvNet.datasets.generate_dataset import CenterCropGenerator
from UltraUPConvNet.datasets.generate_dataset import USdatasetCls, USdatasetSeg

from utils import seg_test
from sklearn.metrics import accuracy_score

from networks.UltraUPConvNet import UltraUPConvNet
...
def inference(args, model, test_save_path=None):
    ...
    result_csv_path = os.path.join(args.output_dir, "result.csv")
    ...
    for dataset_name in seg_test_set:
        ...
        with open(result_csv_path, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            if args.prompt:
                writer.writerow([dataset_name, 'seg_prompt@'+args.output_dir, performance,
                                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())])
            else:
                writer.writerow([dataset_name, 'seg@'+args.output_dir, performance,
                                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())])
    ...
    for dataset_name in cls_test_set:
        ...
        with open(result_csv_path, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            if args.prompt:
                writer.writerow([dataset_name, 'cls_prompt@'+args.output_dir, performance,
                                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())])
            else:
                writer.writerow([dataset_name, 'cls@'+args.output_dir, performance,
                                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())])
