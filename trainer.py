...
from datasets.generate_dataset import USdatasetCls, USdatasetSeg
from datasets.get_dataset import WeightedRandomSamplerDDP
from datasets.get_dataset import USdataset_cls, USdataset_seg
from datasets.generate_dataset import RandomGenerator, CenterCropGenerator
from sklearn.metrics import roc_auc_score
from utils import seg_test

import torch.nn.functional as F


def train(args, model, snapshot_path):
    ...
    db_train_seg = USdataset_seg(base_dir=args.root_path, split="train", transform=transforms.Compose(
    ...
    db_train_cls = USdataset_cls(base_dir=args.root_path, split="train", transform=transforms.Compose(
        [RandomGenerator(output_size=[args.img_size, args.img_size])]), prompt=args.prompt)
    ...
    #     # 将两个 dataloader 合并为一个循环
    #     for i_batch, (sampled_batch_seg, sampled_batch_cls) in tqdm(enumerate(zip(trainloader_seg, trainloader_cls))):
    #         # ----------------------
    #         # 1. 分割任务输入处理
    #         # ----------------------
    #         image_seg, label_seg = sampled_batch_seg['image'], sampled_batch_seg['label']
    #         image_seg, label_seg = image_seg.to(device), label_seg.to(device)

    #         # ----------------------
    #         # 2. 分类任务输入处理
    #         # ----------------------
    #         image_cls, label_cls = sampled_batch_cls['image'], sampled_batch_cls['label']
    #         num_classes_batch = sampled_batch_cls['num_classes']
    #         image_cls, label_cls = image_cls.to(device), label_cls.to(device)
    ...
    #                 return (
    #                     torch.tensor(np.array(batch['position_prompt'])).permute([1, 0]).float().to(device),
    #                     torch.tensor(np.array(batch['task_prompt'])).permute([1, 0]).float().to(device),
    #                     torch.tensor(np.array(batch['type_prompt'])).permute([1, 0]).float().to(device),
    #                     torch.tensor(np.array(batch['nature_prompt'])).permute([1, 0]).float().to(device)
    #                 )

    #             pos_prompt_seg, task_prompt_seg, type_prompt_seg, nature_prompt_seg = get_prompt(sampled_batch_seg)
    #             pos_prompt_cls, task_prompt_cls, type_prompt_cls, nature_prompt_cls = get_prompt(sampled_batch_cls)

    #             # 假设 model 内部能根据任务类型处理 prompt（否则建议合并处理）
    #             (x_seg, _, _) = model((image_seg, pos_prompt_seg, task_prompt_seg, type_prompt_seg, nature_prompt_seg))
    #             (_, x_cls_2, x_cls_4) = model((image_cls, pos_prompt_cls, task_prompt_cls, type_prompt_cls, nature_prompt_cls))
    #         else:
    #             (x_seg, _, _) = model(image_seg)
    #             (_, x_cls_2, x_cls_4) = model(image_cls)
    ...
    #         # 分割 loss
    #         loss_seg_ce = seg_ce_loss(x_seg, label_seg.long())
    #         loss_seg_dice = seg_dice_loss(x_seg, label_seg, softmax=True)
    #         loss_seg = 0.4 * loss_seg_ce + 0.6 * loss_seg_dice
    ...
    #         if mask_2_way.any():
    #             outputs_2_way = x_cls_2[mask_2_way]
    #             labels_2_way = label_cls[mask_2_way]
    #             loss_cls_2 = cls_ce_loss_2way(outputs_2_way, labels_2_way.long())
    #             loss_cls += loss_cls_2

    #         if mask_4_way.any():
    #             outputs_4_way = x_cls_4[mask_4_way]
    #             labels_4_way = label_cls[mask_4_way]
    #             loss_cls_4 = cls_ce_loss_4way(outputs_4_way, labels_4_way.long())
    #             loss_cls += loss_cls_4

    #         # ----------------------
    #         # 5. 总 loss & 反向传播
    #         # ----------------------
    #         total_loss = 10 * loss_seg + loss_cls  # 可调权重
    #         optimizer.zero_grad()
    #         total_loss.backward()
    #         optimizer.step()
    ...
    #         writer.add_scalar('info/seg_loss', loss_seg.item(), seg_iter_num)
    #         writer.add_scalar('info/cls_loss', loss_cls.item(), cls_iter_num)

    #         logging.info('global iteration %d: seg_loss = %.4f, cls_loss = %.4f, total = %.4f' %
    #                     (global_iter_num, loss_seg.item(), loss_cls.item(), total_loss.item()))
        ...
        for dataset_name in seg_val_set:
            ...
            for i_batch, sampled_batch in tqdm(enumerate(val_loader)):
                ...
                if args.prompt:
                    ...
                    metric_i = seg_test(image, label, model,
                                                classes=num_classes,
                                                prompt=args.prompt,
